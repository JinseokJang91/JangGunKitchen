...
OAuth 콜백 로컬 포트는 **8080** (`backend/app`). 콘솔 redirect URI와 맞출 것.
...
| 호출 주체 | 본 시스템 → 외부 시스템 중심 (예외: OAuth 콜백 엔드포인트 수신) |
| 사용 목적 | 소셜 로그인(OAuth 2.0): 토큰 발급 및 사용자 정보 조회 |

본 애플리케이션은 **외부로 나가는 호출**만 있으며, 네이버/구글/카카오 OAuth 서버에 토큰·사용자정보 요청을 보냅니다.  
외부 시스템이 본 애플리케이션의 비즈니스 API를 직접 호출하는 수신 인터페이스는 없습니다.  
단, OAuth 인증 완료 시 브라우저 리다이렉트를 통해 백엔드의 콜백 엔드포인트(`/api/auth/{provider}/callback`)로 진입합니다.
...
| No | 연동 대상 | 방향 | 용도 | 비고 |
|----|-----------|------|------|------|
| 1 | 네이버 로그인(NAVER OAuth 2.0) | 송신 | 액세스 토큰 발급, 사용자 정보 조회 | backend/app auth |
| 2 | 구글 로그인(Google OAuth 2.0) | 송신 | 액세스 토큰 발급, 사용자 정보 조회 | backend/app auth |
| 3 | 카카오 로그인(Kakao OAuth 2.0) | 송신 | 액세스 토큰 발급, 사용자 정보 조회 | backend/app auth |
...
| 방향 | 본 시스템 → 네이버 (송신) |
| 구현 위치 | `backend/app` / `NaverAuthService` |
...
| 방향 | 본 시스템 → 구글 (송신) |
| 구현 위치 | `backend/app` / `GoogleAuthService` |
| 전송 방식 | RestTemplate, application/x-www-form-urlencoded / Bearer Token |
...
| 방향 | 본 시스템 → 카카오 (송신) |
| 구현 위치 | `backend/app` / `KakaoAuthService` |
| 전송 방식 | RestTemplate, application/x-www-form-urlencoded / Bearer Token |
...
| No | 연동 대상 | 용도 | 비고 |
|----|-----------|------|------|
| 1 | OAuth 제공자(브라우저 리다이렉트) | OAuth 인가코드 전달(콜백) | `/api/auth/naver/callback`, `/api/auth/google/callback`, `/api/auth/kakao/callback` |
